 ...
 */
size_t encode_frame(const int16* input, int8* output, size_t frame_len) {
  const int16* source    = input;
  size_t       count     = frame_len;
  int16        max_14bit = 0;

  /* Identify the loudest 14-bit sample value */
  while (count--) {
    int16 cur_14bit = (*source++) >> 2;
    int16 abs_14bit = cur_14bit < 0         ? -cur_14bit : cur_14bit;
    max_14bit       = abs_14bit > max_14bit ?  abs_14bit : max_14bit;
  }

  /* If we comnputed a non-zero maximum 14-bit sample, work out the best AM volume to use and convert the frame */
  if (max_14bit) {
    float32 ideal_factor = 8192.0 / (float32)max_14bit;
    ...
    best_factor = amp_factors[amp_index];
    int max_8bit = (int)(max_14bit * best_factor) >> 6;
